...
namespace Classes.Hexes
{
    public class TwoAndThree : Hex
    {
        ...
        public TwoAndThree() : base("TwoAndThree")
        {
            HexEffect = (_, target, isCrit) =>
            {
                ...
                if (count >= 2)
                {
                    ...
                    bullet.OnBulletHit += (self) =>
                    {
                        // 造成200%的攻击伤害和攻击特效
                        // 计算攻击伤害
                        var damageCount = target.CalculateADDamage(owner, owner.attackDamage) * 2;
                        ...
                        if (target.isAlive)
                        {
                            owner.AttackEffectActivate(target, damageCount, 2);
                        }
                    };
