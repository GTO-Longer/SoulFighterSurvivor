...
{
    public class RavenousHydra : Equipment
    {
        ...
        private float activeDamageCount => 1.3f * HeroManager.hero.attackDamage;
        
        public RavenousHydra() : base("RavenousHydra")
        {
            PassiveEquipmentEffect += (_, target, _, _) =>
            {
                if (target.isAlive)
                {
                    ...
                    damageArea.OnBulletAwake += (self) =>
                    {
                        ...
                        self.effect = EffectManager.Instance.CreateEffect("RavenousHydra", self.gameObject);
                        ...
                        if (targets != null)
                        {
                            foreach (var entity in targets)
                            {
                                ...
                                if (self.bulletEntityDamageCDTimer[entity] > self.bulletDamageCD)
                                {
                                    ...
                                    owner.TakeHeal(damageValue * owner.lifeSteal);
                                }
                            }
                        }
